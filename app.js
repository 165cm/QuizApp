...
- このテキストの主要なテーマは何か？
- 学習者が身につけるべき知識・スキルは何か？
- どのような文脈で使われる知識か？（例：日常会話、技術文書、学術論文）

**重要**:
- 専門用語、固有名詞、外国語の単語は**必ず原語を保持**すること
- カタカナ変換は絶対に行わないこと
- 例：「Kenot」→「ケノット」は❌、「Kenot」のまま保持すること✅

【手順2: 問題生成】
学習目的に基づいて、以下の方針で問題を作成してください。

## 問題文の作成方針:
### 必須要素（すべての問題に含めること）:
1. **学習目的との整合性**: 手順1で特定した学習目的に沿った問題にする
2. **具体的な使用例・応用場面**: 実際にどのような場面で使われるかを示す
   - 例：「マレーシアで友達と会話中に...」「レストランで注文する時に...」
3. **記憶のフック**: 覚えやすいイメージ、語呂合わせ、ストーリー
4. **原語の保持**: 外国語の単語は必ず原語のまま表記する
   - 例：「Tapau」「Kenot」「Adoi」など
   - カタカナ変換は絶対に行わない

### 問題文の構成（3-4文）:
- 1文目: 具体的な使用シーン（いつ、どこで、誰が使うか）
- 2文目: 覚えやすいイメージや文化的背景
- 3文目: 実際の問い
- （オプション）ヒント: 意味や由来のヒント

例：❌「「ケノット」という言葉はどのような意味か？」（カタカナ変換されている）
    ✅「マレーシアで「できない」を伝える時によく使われるスラング。標準マレー語の「tidak boleh」を短縮したもので、若者の間で頻繁に使われます。この言葉は何でしょう？（ヒント：否定を表す言葉です）」

## 選択肢の作成方針:
- **原語を保持**: 必ず原語のまま表記（例：「Kenot」のまま）
- カタカナは絶対に使用しない
- 「（正解）」などの説明は含めない
- 実際によくある誤解や混同しやすい言葉を選択肢に

例：
✅ ["Kenot", "Boleh", "Tak nak", "Mesti"]  （原語保持）
❌ ["ケノット", "ボレ", "タック・ナック", "メスティ"]  （カタカナ変換）

## 解説文の作成方針:
### 必須要素:
1. **正解の原語表記**: 解説でも必ず原語を使用
2. **語源や文化的背景**: なぜそう言うのか、どこから来た言葉か
3. **実践的な使用例**: 実際の会話での使い方
4. **次の疑問への先回り回答**: 関連する表現や注意点
5. **文字数制限**: 200-300文字厳守（10秒で読み切れる量）
...
出力形式:
{
  "learningObjective": "この教材の学習目的（1-2文）",
  "context": "使用される文脈（例：マレーシア日常会話、プログラミング基礎、など）",
  "sections": [
    {
      "heading": "見出し1",
      "level": 1
  ...
  "questions": [
    {
      "question": "学習目的に沿った問題文（原語保持、3-4文）",
      "choices": ["原語1", "原語2", "原語3", "原語4"],
      "correctIndex": 0,
      "explanation": "深い洞察を含む解説文（原語保持、200-300文字厳守）",
      "difficulty": "basic",
      "sourceSection": "見出し1",
...
注意事項:
- **最重要**: 外国語の単語は絶対にカタカナ変換しないこと
- 問題文、選択肢、解説すべてで原語を保持すること
- 学習目的を明確にし、それに沿った問題を作成すること
- 解説は200-300文字厳守（10秒で読み切れる量）
